...
use crate::auth::service::{BootstrapOwnerError, LoginError};
...
pub enum AuthHttpError {
    BootstrapOwner(BootstrapOwnerError),
    Login(LoginError),
}
...
impl From<BootstrapOwnerError> for AuthHttpError {
    ...
    fn from(error: BootstrapOwnerError) -> Self {
        Self::BootstrapOwner(error)
    }
}

impl From<LoginError> for AuthHttpError {
...
impl IntoResponse for AuthHttpError {
    ...
    fn into_response(self) -> Response {
        match self {
            ...
            AuthHttpError::Login(err) => {
                let (status, message) = match err {
                    LoginError::InvalidCredentials => {
                        (StatusCode::UNAUTHORIZED, "用户名或密码错误")
                    }

                    LoginError::PasswordVerification(_)
                    | LoginError::Database(_)
                    | LoginError::PasswordTask(_) => {
